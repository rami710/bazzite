...
RUN --mount=type=cache,dst=/var/cache \
    ...
    /ctx/build-initramfs && \
    /ctx/finalize
    # --- AIR OS: ARCHITECTURE ULTRA-LÉGÈRE ---

# Suppression radicale des services inutiles
RUN rpm-ostree override remove \
    hp-health hp-wmi avahi-daemon thermald \
    PackageKit PackageKit-command-not-found \
    gnome-software firefox

# Installation des dépendances pour AirAI
RUN rpm-ostree install python3-pip python3-tkinter speech-recognition
